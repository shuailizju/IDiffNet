...
def DenseNetFCN(input_shape, nb_dense_block=5, growth_rate=16, nb_layers_per_block=4,
                ...
                include_top=True, weights=None, input_tensor=None, classes=1, activation='relu',
...
def __conv_block(ip, nb_filter, bottleneck=False, dropout_rate=None, weight_decay=1E-4):
    ...
    x = Conv2D(nb_filter, (5, 5), kernel_initializer='he_uniform', padding='same', use_bias=False, dilation_rate=2,
               kernel_regularizer=l2(weight_decay))(x)
    if dropout_rate:
        x = Dropout(dropout_rate)(x)
...
def __create_fcn_dense_net(nb_classes, img_input, include_top, nb_dense_block=5, growth_rate=12,
    ...
    # Initial convolution
    img_input=Permute((2,3,1))(img_input)
    x = Conv2D(init_conv_filters, (5, 5), kernel_initializer='he_uniform',padding='same', name='initial_conv2D',dilation_rate=2,
               use_bias=False, kernel_regularizer=l2(weight_decay))(img_input)
    x = AveragePooling2D((2, 2), strides=(2, 2))(x)
    ...
    #add one more transition up block
    l = concatenate(concat_list[1:], axis=concat_axis)

    x_up = __transition_up_block(l, nb_filters=n_filters_keep, type=upsampling_type)       
    ...
    if include_top:
        ...
        #x = Reshape((row * col//4, nb_classes))(x)
        x = Activation(activation)(x)
        x = Permute((3,1,2))(x)
        #x = Reshape((row//2, col//2, nb_classes))(x)
    else:
        x = x_up
